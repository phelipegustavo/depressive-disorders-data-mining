...
ENTREZ = 'https://eutils.ncbi.nlm.nih.gov/entrez'
...
def get_iter_tex(tree):
    if tree is not None:
        text = [t for t in tree.itertext()]
        text = [t.replace('\n', ' ').replace('\t', ' ') for t in text]
        return ' '.join(text)
    return ''

def get_text(root, path, separator=' '):
    try:
        tree = root.findall(path)
        if tree is not None:
            return separator.join([t.text for t in tree])
        return ''
    except:
        return ''

def get_iter_tex_dic(root, path):
    tree = root.findall(path)
    dic = {}
    if tree is not None:
        for i in tree:
            dic[i.tag] = i.text.replace('\n', ' ').replace('\t', ' ')
    return dic

def get_text_array(root, path):
    tree = root.findall(path)
    arr = []
    if tree is not None:
        for i in tree:
...
def get_atrib_dic(root, path, atrib):
    tree = root.findall(path)
    dic = {}
    if tree is not None:
        for i in tree:
            dic[i.attrib[atrib]] = i.text
    return dic